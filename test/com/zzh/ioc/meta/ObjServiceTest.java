...
import com.zzh.ioc.meta.fake.Faking;
...
public class ObjServiceTest extends DaoCase {
	...
	protected void before() {
		...
		SqlManager sqls = new FileSqlManager("com/zzh/ioc/dbtest.sqls");
		dao.execute(sqls.createComboSql());
	}
	...
	public void testFetchObject() {
		...
		assertEquals("com.zzh.dao.impl.FileSqlManager", sqls.getType());
		assertTrue(sqls.isSingleton());
		assertNull(sqls.getDeposeby());
		assertNull(sqls.getDeposer());
		...
		assertEquals("com.zzh.fake.FakeKeys.keys", sqls.getFields()[0].getVal().getValue());

		obj = srv.fetchObject("colors");
		assertNull(obj.getComment());
		assertEquals("com.zzh.fake.ColorDeposer", obj.getDeposer());
		assertTrue(obj.getArgs()[0].isRefer());
		assertEquals("dao", obj.getArgs()[0].getValue());
		assertTrue(obj.getArgs()[1].isDisk());
		assertEquals("com/zzh/mole/Mole.class", obj.getArgs()[1].getValue());
		...
		assertEquals("com.zzh.Static.name", flds.get("callback").getVal().getValue());
