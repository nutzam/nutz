...
public class LoadingsImpl extends Loadings{
	...
	public static void evalAt(ActionInfo ai, At at, Class<?> clazz) {
		...
		if (null != at) {
			...
			if(clazz.getAnnotation(Modules.class) == null && def.endsWith("Controller")){// 只处理符合规范的控制器
				String pkg = clazz.getPackage().getName();
				int start = pkg.indexOf("controllers");
				if(start != -1){// 严格按照规范的处理子包
					if(pkg.length() == start+"controllers".length())
						pkg = "/";
					else
						pkg = pkg.substring(start+"controllers".length()).replaceAll(".", "/");
					String ctrName = Strings.lowerFirst(def.substring(0,def.length()-"Controller".length()));
					ai.setPaths(Lang.array(pkg + ctrName));
				}else{// 不按照规范的不鼓励，以默认包为准
					String ctrName = Strings.lowerFirst(def.substring(0,def.length()-"Controller".length()));
					ai.setPaths(Lang.array("/"+ctrName));
				}
			}
		}
	}
