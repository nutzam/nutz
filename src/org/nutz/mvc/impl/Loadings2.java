...
import org.nutz.mvc.annotation.Scoffold;
import org.nutz.resource.Scans;

public class Loadings2{
	...
	public static ActionInfo createInfo(Method method,Class<?> module) {
		...
		evalOk(ai, method.getAnnotation(Ok.class), method,module);
	...
	public static void evalOk(ActionInfo ai, Ok ok, Method action ,Class<?> clazz) {
		if (null != ok) {
			ai.setOkView(ok.value());
		} else {// create by wangc
			String def = action.getName();
			if(isController(clazz)){
				String pre = "jsp";
				if(null != action.getAnnotation(Scoffold.class)){
					pre = "vm";
				}
				String moduleName = getContrllerModule(clazz);
				String ctnName = getControllerName(clazz);
				if(Strings.isEmpty(moduleName)){
					ai.setOkView(pre+":views."+ ctnName+"."+Strings.lowerFirst(def));
				}else{
					ai.setOkView(pre+":views."+moduleName+"."+ctnName+"."+Strings.lowerFirst(def));
				}
			}else{
				if(clazz.getAnnotation(Modules.class) != null){
