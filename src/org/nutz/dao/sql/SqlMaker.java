...
public class SqlMaker {
	...
	public Sql insert_manymany(Link link) {
		return Sqls.create(format(	"INSERT INTO %s (%s,%s) VALUES(@%s,@%s)",
									link.getRelation(),
									link.getFrom(),
									link.getTo(),
									link.getFrom(),
	...
	public Sql update(Entity<?> en, Object obj) {
		...
		// 这个 POJO 用的是单一主键
		if (null != idf) {
			String fmt = format("UPDATE %s SET %s WHERE %s=@%s",
								en.getTableName(),
								sb,
								idf.getColumnName(),
								idf.getFieldName());
		...
		if (null == pks || pks.length <= 1) {
			throw Lang.makeThrow(	"You should define @Id or @Name or @PK for POJO '%s'",
									en.getType());
		}
		// 从 POJO 中获取复合主键的值
	...
	public Sql fetch(Entity<?> entity, EntityField ef) {
		...
		if (ef == null) {
			...
			fmt = format(	"SELECT %s FROM %s WHERE LOWER(%s)=LOWER(@%s)",
							fields,
							entity.getViewName(),
							ef.getColumnName(),
							ef.getFieldName());
