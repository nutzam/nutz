...
public abstract class Daos {
	...
	public static void checkTableColumn(Dao dao, Object tableName, final Class<?> clsType) {
		...
		ext(d, tableName).run(new ConnCallback() {
			public void invoke(Connection conn) throws Exception {
				...
				d.getJdbcExpert().setupEntityField(conn, en);
			}
		});
	}
	 ...
	 *            是否允许删除
	 */
	public static void migration(Dao dao, final Class<?> klass, final boolean add, final boolean del, Object tablename) {
	 ...
	 *            是否允许删除
	 */
	public static void migration(Dao dao, String packageName, boolean add, boolean del, String tablename) {
		for (Class<?> klass : Scans.me().scanPackage(packageName)) {
			if (klass.getAnnotation(Table.class) != null) {
				migration(dao, klass, add, del, tablename);
			}
		}
	}
