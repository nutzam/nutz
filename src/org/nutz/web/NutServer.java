...
import org.nutz.web.impl.action.FileAccessAction;
...
public class NutServer implements Runnable {
	...
	public void shutdown() {
	   ...
	   public void _run() throws IOException {
	        ...
	        InetSocketAddress address = new InetSocketAddress(conf().getAppPort());
	                ...
	                while (iterator.hasNext()) {
	                    ...
	                    if (selectionKey.isAcceptable()) { //新连接!!
	                        ...
	                        if (pos > 0) { //看来请求头拿到了!!
	                            ...
	                            workFor(socket, head, preRead);//交给容器处理了, 我不管了!!
	                        }
		...
		es.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					if (action == null) {
					    log.debug("None NutHttpAction for req--> " + req.requestURI());
					    socket.close();
					    return;
					}
	...
	protected NutHttpAction defaultHttpAction;
	...
	public static void main(String[] args) {
		...
		server.defaultHttpAction = new FileAccessAction(conf.getAppRoot());
