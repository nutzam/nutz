	...
	<T> T parseFromJson(InputStream ins, Class<T> type) {
		...
		try {
			...
			case '[':
				...
				if (!type.isArray()) {
					throw new RuntimeException(String.format("type should be a Array, not '%s'",
							type.getName()));
				}
				Class<?> componentType = type.getComponentType();
				List list = new LinkedList();
				do {
					Object o = parseFromJson(ins, componentType);
					list.add(o);
					// find next element
				...
				if (Object.class.isAssignableFrom(componentType))
					return (T) list.toArray((Object[]) ary);
				int i = 0;
				for (Iterator it = list.iterator(); it.hasNext();)
					Array.set(ary, i++, it.next());
				return (T) ary;
				// Object or Map
			case '{':
				/*
			...
			case '9':
				...
				if (null == me) { // guess the return type
					...
					else if (Character.toUpperCase(numValue.charAt(numValue.length() - 1)) == 'L') {
						String vv = numValue.substring(0, numValue.toUpperCase().lastIndexOf('L'));
						return (T) Long.valueOf(vv);
					} else
